...
function templatePath() {
  const custom = process.env.CERTIFICATE_TEMPLATE_PATH;
  if (custom && fs.existsSync(custom)) return custom;
  return path.join(__dirname, "..", "assets", "certificate-template.pdf");
}
...
async function generateCertificatePdf(opts) {
  ...
  const courseName = String(opts.courseName || "").trim();
  const tpl = templatePath();
  if (!fs.existsSync(tpl)) {
    throw new Error(
      "Certificate template missing. Run: npm run cert:build-template"
    );
  }
...
module.exports = { generateCertificatePdf, sanitizeFilename, templatePath };
