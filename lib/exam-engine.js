...
async function submitAttempt(attemptId, candidateId, candidateRow, opts = {}) {
  ...
  const emailFields = {
    ...
    examTitle: exam.title,
    courseName: exam.course_name || candidateRow?.course_name || exam.title,
    scorePercent,
    cutoffPercent: exam.cutoff_percent,
    batch: String(candidateRow?.batch || "").trim(),
    agentName: String(candidateRow?.company_name || "").trim()
  };

  if (submitReason === "tab_switch" && candidateRow?.email) {
    try {
      await sendExamIntegrityEmail({
      ...
      console.error("Exam integrity email failed:", e.message);
    }
  } else if (passed) {
