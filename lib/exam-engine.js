...
const { newId, MAX_INTEGRITY_EVENTS, TIMER_GRACE_MS } = require("./security");
...
const { courseMatches, resolveCatalogCourse } = require("./courses");

const DEFAULT_ATTEMPTS_MAX = 2;
const MAX_ATTEMPTS_PER_EXAM = DEFAULT_ATTEMPTS_MAX;

function examAttemptsMax(exam) {
  const n = parseInt(exam?.attempts_max, 10);
  if (!Number.isFinite(n)) return DEFAULT_ATTEMPTS_MAX;
  return Math.min(10, Math.max(1, n));
}

function clampAttemptsMax(value) {
...
async function listExhaustedCandidateExams() {
  const pairs = await getDb().all(
    `SELECT DISTINCT a.candidate_id, a.exam_id,
      ...
      e.title AS exam_title, e.attempts_max
...
async function deliverPassCertificate({
  ...
  const now = new Date().toISOString();
  const certId = `PP-CERT-${crypto.randomBytes(16).toString("hex").toUpperCase()}`;
...
function attemptEndsAtMs(row) {
  const started = new Date(row.started_at).getTime();
  const mins = Number(row.duration_minutes ?? 0);
  return started + mins * 60 * 1000;
}

function isAttemptTimedOut(row, now = Date.now()) {
  if (!row?.started_at) return false;
  return now > attemptEndsAtMs(row) + TIMER_GRACE_MS;
}

function resolveSubmitReasonForAttempt(row, clientReason) {
  const integrity = Number(row.integrity_events || 0);
  if (integrity >= MAX_INTEGRITY_EVENTS) return "tab_switch";
  if (isAttemptTimedOut(row)) return "timeout";
  return normalizeSubmitReason(clientReason);
}

async function recordIntegrityEvent(attemptId, candidateId) {
  const db = getDb();
  ...
  assertCandidateExamAccess(candidate, exam);
  if (isAttemptTimedOut(row)) {
    const err = new Error("Exam time has expired.");
    err.status = 403;
    throw err;
  }
  const upd = await db.run(
    `UPDATE exam_attempts
     SET integrity_events = integrity_events + 1
     WHERE id = ? AND candidate_id = ? AND submitted_at IS NULL`,
    [attemptId, candidateId]
  );
  if (!upd.changes) {
    const err = new Error("Attempt not found or already submitted.");
  ...
  const fresh = await db.get("SELECT integrity_events FROM exam_attempts WHERE id = ?", [attemptId]);
  const events = Number(fresh?.integrity_events || 0);
  return {
    ok: true,
    events,
    maxEvents: MAX_INTEGRITY_EVENTS,
    autoSubmit: events >= MAX_INTEGRITY_EVENTS
  };
}
...
function assertCandidateCourseForExam(candidate, exam) {
  if (!exam) {
    const err = new Error("Exam not found.");
    err.status = 404;
    throw err;
  }
  if (!resolveCatalogCourse(candidate?.course_name)) {
    const err = new Error("Your course is not set up for exams. Contact your training partner.");
    err.status = 403;
    throw err;
  }
...
function assertCandidateExamAccess(candidate, exam) {
  if (!exam || exam.status !== "published") {
    const err = new Error("Exam not available.");
    err.status = 404;
    throw err;
  }
  assertCandidateCourseForExam(candidate, exam);
}
...
async function getExamPreview(examId, candidate) {
  ...
  return {
    ...
    attemptsUsed,
    attemptsMax: max,
    canTake: attemptsUsed < max || Boolean(inProgress),
...
async function startAttempt(examId, candidate) {
  ...
  if (attemptsUsed >= max) {
    const err = new Error(
      `You have used all ${max} attempt${max === 1 ? "" : "s"} for this exam.`
    );
    err.status = 409;
    throw err;
  }
  ...
  try {
    ...
    if (msg.includes("unique") || msg.includes("constraint")) {
      ...
      if (raced) {
        const resumed = await resumeAttempt(raced);
        resumed.studentName = String(candidate.name || "").trim();
        return resumed;
      }
    }
...
async function saveAnswer(attemptId, candidateId, questionId, selectedIndex) {
  ...
  if (!questionOrder.includes(qid)) {
    const err = new Error("Invalid question for this attempt.");
    err.status = 400;
    throw err;
  }
  const optionMaps = JSON.parse(row.option_maps_json || "{}");
  ...
  if (!Number.isInteger(idx) || idx < 0 || idx >= map.length) {
    const err = new Error("Invalid answer selection.");
    err.status = 400;
    throw err;
  }
  const answers = JSON.parse(row.answers_json || "{}");
...
async function submitAttempt(attemptId, candidateId, candidateRow, opts = {}) {
  ...
  if (row.submitted_at) {
    assertCandidateCourseForExam(candidateRow, exam);
  ...
  assertCandidateExamAccess(candidateRow, exam);
  const questionOrder = JSON.parse(row.question_order_json || "[]");
  const optionMaps = JSON.parse(row.option_maps_json || "{}");
  const answers = JSON.parse(row.answers_json || "{}");
  ...
  const scorePassed = scorePercent >= Number(exam.cutoff_percent || 0) ? 1 : 0;
  const submitReason = resolveSubmitReasonForAttempt(row, opts.submitReason);
  const passed = submitReason === "tab_switch" ? 0 : scorePassed;
  const now = new Date().toISOString();

  const submitUpdate = await db.run(
    `UPDATE exam_attempts
  ...
  if (submitReason === "tab_switch" || !passed) {
    const mail = await deliverAttemptResultEmails({
      ...
      candidateRow,
      scorePercent,
      passed: submitReason === "tab_switch" ? !!scorePassed : !!passed,
      submitReason,
      skipIfSent: false
    });
  ...
  return {
    ...
    attemptsMax: examAttemptsMax(exam),
    submittedAt: now
  };
}
