...
function courseMatches(candidateCourse, examCourse) {
  const a = String(candidateCourse || "").trim().toLowerCase();
  const b = String(examCourse || "").trim().toLowerCase();
  return a && b && a === b;
}
...
async function startAttempt(examId, candidate) {
  ...
  await db.run(
    `INSERT INTO exam_attempts
     (id, exam_id, candidate_id, started_at, question_order_json, option_maps_json, answers_json)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [
      attemptId,
      examId,
  ...
  return buildAttemptPayload(attemptId, exam, questionOrder, optionMaps, {}, now, null);
}
