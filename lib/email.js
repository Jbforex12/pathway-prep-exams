...
const { otpEmail, examCertificateEmail } = require("./email-templates");
...
async function sendExamCertificateEmail({ to, studentName, examTitle, scorePercent, pdfBuffer }) {
  const cfg = getEmailConfig();
  const tpl = examCertificateEmail({ studentName, examTitle, scorePercent, brand: cfg.brand });
  return sendMail({
    to,
    ...tpl,
    attachments: [{ filename: "exam-certificate.pdf", content: pdfBuffer }]
  });
}

module.exports = { sendOtpEmail, sendExamCertificateEmail };
